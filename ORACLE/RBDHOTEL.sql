...
BEGIN
  ...
  IF V_LICZBA_OSOB_POKOJ <= 0 THEN
    RAISE_APPLICATION_ERROR(-20016, 'LICZBA OSÓB W POKOJU MUSI BYÆ WIÊKSZA OD ZERA');
  END IF;
  ...
  FOR R IN (SELECT ID_USLUGI, LICZBA_OSOB FROM REZERWACJE_USLUGI WHERE ID_REZERWACJI = P_ID_REZERWACJI) LOOP
    SELECT CENA INTO V_CENA_USLUGA FROM USLUGI WHERE ID_USLUGI = R.ID_USLUGI;
    IF V_CENA_USLUGA < 0 THEN
      RAISE_APPLICATION_ERROR(-20008, 'CENA JEST B£ÊDNA, POWINNA BYÆ WIÊKSZA OD ZERA');
    END IF;

    IF R.LICZBA_OSOB < 0 OR R.LICZBA_OSOB > V_LICZBA_OSOB_POKOJ THEN
    ...
    V_CENA_POBYTU := V_CENA_POBYTU + (R.LICZBA_OSOB * V_CENA_USLUGA);
  END LOOP;

  V_CENA_POBYTU := ((V_LICZBA_DNI * V_CENA_POKOJU) + V_OPLATA_ZNISZCZENIE + V_CENA_POBYTU) * (1 - (V_PROCENT_ZNIZKI / 100));
