...
BEGIN
  IF :NEW.PROCENT < 0 AND :NEW.PROCENT > 100 THEN
    RAISE_APPLICATION_ERROR(-20019, 'PROCENT PWOINIEN BY∆ W ZAKRESIE [0, 100]');
  END IF;
...
ID_USLUGI NUMBER;
CENA_USLUGA NUMBER;
LICZBA_OSOB_USLUGA NUMBER;
LICZBA_OSOB_POKOJ NUMBER;
TEMP NUMBER;
BEGIN
  
  SELECT COUNT(*) INTO TEMP FROM REZERWACJE WHERE ID_REZERWACJI = ID_REZERWACJI;
  IF TEMP = 0 THEN
  ...
  IF LICZBA_DNI <= 0 THEN
    RAISE_APPLICATION_ERROR(-20005, 'REZERWACJA NIE TRWA£A NAWET JEDEN DZIE—');
  END IF;
    
  SELECT OPLATA_ZNISZCZENIE INTO OPLATA_ZNISZCZENIE FROM REZERWACJE WHERE ID_REZERWACJI = ID_REZERWACJI;
  IF OPLATA_ZNISZCZENIE < 0 AND OPLATA_ZNISZCZENIE IS NOT NULL THEN
    RAISE_APPLICATION_ERROR(-20006, 'B£ DNA WARTOå∆ OP£ATA_ZNISZCZENIE');
  END IF;
  
  SELECT ID_POKOJU INTO ID_POKOJU FROM REZERWACJE WHERE ID_REZERWACJI = ID_REZERWACJI;
  SELECT COUNT(*) INTO TEMP FROM POKOJE WHERE ID_POKOJU = ID_POKOJU;
  IF TEMP = 0 THEN
    RAISE_APPLICATION_ERROR(-20007, 'POK”J NIE ISTNIEJE');
  END IF;
  
  SELECT CENA INTO CENA_POKOJU FROM POKOJE WHERE ID_POKOJU = ID_POKOJU;
  IF CENA_POKOJU <= 0 THEN
    RAISE_APPLICATION_ERROR(-20008, 'CENA JEST B£ DNA, POWINNA BY∆ WI KSZA OD ZERA');
  END IF;
  
  SELECT LICZBA_OSOB INTO LICZBA_OSOB_POKOJ FROM POKOJE WHERE ID_POKOJU = ID_POKOJU;
  IF LICZBA_OSOB_POKOJ <> 2 AND LICZBA_OSOB_POKOJ <> 3 AND LICZBA_OSOB_POKOJ <> 4 THEN
    RAISE_APPLICATION_ERROR(-20016, 'LICZBA OS”B W POKOJU MUSI BY∆ 2, 3 LUB 4');
  END IF;
  
  SELECT ID_KLIENTA INTO ID_KLIENTA FROM REZERWACJE WHERE ID_REZERWACJI = ID_REZERWACJI;
  SELECT COUNT(*) INTO TEMP FROM KLIENCI WHERE ID_KLIENTA = ID_KLIENTA;
  IF TEMP = 0 THEN
  ...
  SELECT COUNT(*) INTO TEMP FROM ZNIZKI WHERE ID_ZNIZKI = ID_ZNIZKI;
  IF TEMP = 0 THEN
    RAISE_APPLICATION_ERROR(-20010, 'ZNIØKA NIE ISTNIEJE');
  END IF;
  
  SELECT PROCENT INTO PROCENT_ZNIZKI FROM ZNIZKI WHERE ID_ZNIZKI = ID_ZNIZKI;
  IF PROCENT_ZNIZKI <> 0 AND PROCENT_ZNIZKI <> 5 AND PROCENT_ZNIZKI <> 10 AND PROCENT_ZNIZKI <> 20 THEN
    RAISE_APPLICATION_ERROR(-20011, 'NIEPOPRAWNE WARTOåCI ZNIØKI');
  END IF;
  
  SELECT ID_USLUGI INTO ID_USLUGI FROM REZERWACJE_USLUGI WHERE ID_REZERWACJI = ID_REZERWACJI;
  IF ID_USLUGI <> NULL THEN
    SELECT CENA INTO CENA_USLUGA FROM USLUGI WHERE ID_USLUGI = ID_USLUGI;
    IF CENA_USLUGA < 0 THEN
      RAISE_APPLICATION_ERROR(-20008, 'CENA JEST B£ DNA, POWINNA BY∆ WI KSZA OD ZERA');
    END IF;
  END IF;
  
  SELECT LICZBA_OSOB INTO LICZBA_OSOB_USLUGA FROM REZERWACJE_USLUGI WHERE ID_REZERWACJI = ID_REZERWACJI;
  IF LICZBA_OSOB_USLUGA < 0 AND LICZBA_OSOB_USLUGA > LICZBA_OSOB_POKOJ THEN
    RAISE_APPLICATION_ERROR(-20020, 'LICZBA OSOB W USLUDZE NIE MOZE BYC WIEKSZA NIZ LICZBA OSOB W POKOJU I NIE MOZE BYC MNIEJSZA OD ZERA');
  END IF;
  
  RETURN ((LICZBA_DNI * CENA_POKOJU) + OPLATA_ZNISZCZENIE + (LICZBA_OSOB_USLUGA * CENA_USLUGA)) * (1 - PROCENT_ZNIZKI / 100);
...
BEGIN
  ...
  WHERE ULICA=ULICA AND NR_DOMU=NR_DOMU AND NR_MIESZKANIA=NR_MIESZKANIA AND 
  MIEJSCOWOSC=MIEJSCOWOSC AND KOD_POCZTOWY=KOD_POCZTOWY AND KRAJ=KRAJ AND ROWNUM = 1));
...
BEGIN
  ...
  VALUES((SELECT ID_OSOBY FROM DANE_PERSONALNE WHERE IMIE=IMIE AND NAZWISKO=NAZWISKO 
  AND EMAIL=EMAIL AND NR_TELEFONU=NR_TELEFONU), 1);
END DODAJ_KLIENTA;
/
-------------- ---------------------- -------------- 
-------------- -- DODAJ PRACOWNIKA -- -------------- 
...
BEGIN
  ...
  VALUES((SELECT ID_OSOBY FROM DANE_PERSONALNE WHERE IMIE=IMIE 
  AND NAZWISKO=NAZWISKO AND EMAIL=EMAIL AND NR_TELEFONU=NR_TELEFONU AND ROWNUM=1), ID_HOTELU, SYSDATE, PENSJA, STANOWISKO);
...
TEMP NUMBER;
BEGIN
  SELECT COUNT(*) INTO TEMP FROM PERSONEL WHERE ID_PRACOWNIKA = ID_PRACOWNIKA;
  IF TEMP = 0 THEN
    RAISE_APPLICATION_ERROR(-20012, 'PRACOWNIK NIE ISTNIEJE');
  END IF;
  
  IF PENSJA <= 0 THEN
    RAISE_APPLICATION_ERROR(-20013, 'PENSJA MUSI BYC WI KSZA OD ZERA');
  END IF;
...
BEGIN
  ...
  IF STANOWISKO <> 'SZEF' AND STANOWISKO <> 'POKOJOWKA' AND STANOWISKO <> 'RECEPCJA' AND STANOWISKO <> 'KONSERWATOR' THEN
    RAISE_APPLICATION_ERROR(-20014, 'B£EDNE STANOWISKO MUSI BY∆ SZEF, POKOJOWKA, KONSERWATOR LUB RECEPCJA');
  END IF;
...
BEGIN
  ...
  WHERE ULICA=ULICA AND NR_DOMU=NR_DOMU AND NR_MIESZKANIA=NR_MIESZKANIA 
  AND MIEJSCOWOSC=MIEJSCOWOSC AND KOD_POCZTOWY=KOD_POCZTOWY AND KRAJ=KRAJ AND ROWNUM=1), 
  NAZWA);
END DODAJ_HOTEL;
/
-------------- ---------------- -------------- 
...
TEMP NUMBER;
BEGIN
  SELECT COUNT(*) INTO TEMP FROM HOTELE WHERE ID_HOTELU = ID_HOTELU;
  IF TEMP = 0 THEN
...
BEGIN
  ...
  INSERT INTO GOSCIE VALUES(ID_REZERWACJI, IMIE, NAZWISKO); 
END DODAJ_GOSCIA;
/
-------------- ----------------- -------------- 
-------------- -- USUN GOSCIA -- -------------- 
...
BEGIN
  ...
  IF LICZBA_OSOB <> 2 AND LICZBA_OSOB <> 3 AND LICZBA_OSOB <> 4 THEN
    RAISE_APPLICATION_ERROR(-20016, 'LICZBA OS”B W POKOJU MUSI BY∆ 2, 3 LUB 4');
  END IF;
  ...
  VALUES(ID_HOTELU, NR_POKOJU, LICZBA_OSOB, CENA);
END DODAJ_POKOJ;
/
-------------- ---------------- -------------- 
-------------- -- USUN POKOJ -- -------------- 
...
BEGIN
  ...
  SELECT ID_POKOJU INTO ID_POKOJU FROM REZERWACJE WHERE ID_REZERWACJI = ID_REZERWACJI;
  SELECT COUNT(*) INTO TEMP FROM POKOJE WHERE ID_POKOJU = ID_POKOJU AND NR_POKOJU = NR_POKOJU;
  IF TEMP = 0 THEN
    RAISE_APPLICATION_ERROR(-20007, 'POK”J NIE ISTNIEJE');
  END IF;
  ...
  UPDATE SPRZATANIE SET ID_PRACOWNIKA = ID_PRACOWNIKA, GODZINA_SPRZATANIA = GODZINA 
  WHERE ID_HOTELU = ID_HOTELU AND ID_REZERWACJI = ID_REZERWACJI AND NR_POKOJU = NR_POKOJU;
...
, NR_POKOJU IN NUMBER
, OPIS IN VARCHAR2
, GODZINA_POCZATEK IN TIMESTAMP
, GODZINA_KONIEC IN TIMESTAMP
) AS 
TEMP NUMBER;
ID_POKOJU NUMBER;
BEGIN
  ...
  IF GODZINA_POCZATEK > GODZINA_KONIEC THEN
    RAISE_APPLICATION_ERROR(-20023, 'B£ DNE GODZINY');
  END IF;
  
  INSERT INTO SPRZATANIE(ID_HOTELU, NR_POKOJU, ID_REZERWACJI, OPIS, GODZINY_SPRZATANIA_POCZATEK, GODZINY_SPRZATANIA_KONIEC)
  VALUES(ID_HOTELU, NR_POKOJU, ID_REZERWACJI, OPIS, GODZINA_POCZATEK, GODZINA_KONIEC);
...
BEGIN
  ...
  UPDATE NAPRAWY SET ID_PRACOWNIKA = ID_PRACOWNIKA, GODZINA_NAPRAWY = GODZINA 
  WHERE ID_HOTELU = ID_HOTELU AND ID_REZERWACJI = ID_REZERWACJI AND NR_POKOJU = NR_POKOJU;
...
BEGIN
  ...
  IF ILOSC <= 0 THEN
    RAISE_APPLICATION_ERROR(-20017, 'ILOå∆ WYPOSAØENIA W POKOJU MUSI BY∆ WI KSZA OD ZERA');
  END IF;
  
  INSERT INTO WYPOSAZENIE VALUES(ID_POKOJU, NAZWA, ILOSC); 
...
TEMP NUMBER;
BEGIN
  SELECT COUNT(*) INTO TEMP FROM USLUGI WHERE ID_USLUGI = ID_USLUGI;
  IF TEMP = 0 THEN
...
BEGIN
  ...
  V_CURSOR := SPRAWDZ_WOLNE_POKOJE(ID_HOTELU, LICZBA_OSOB, PRZYJAZD, ODJAZD);
  ...
  INSERT INTO REZERWACJE(ID_HOTELU, ID_KLIENTA, ID_POKOJU, PRZYJAZD, ODJAZD)
  VALUES(ID_HOTELU, ID_KLIENTA, V_ID_POKOJU, PRZYJAZD, ODJAZD);
...
BEGIN
  ...
  IF LICZBA_OSOB < 0 AND LICZBA_OSOB > LICZBA_OSOB_POKOJ THEN
    RAISE_APPLICATION_ERROR(-20020, 'LICZBA OSOB W USLUDZE NIE MOZE BYC WIEKSZA NIZ LICZBA OSOB W POKOJU I NIE MOZE BYC MNIEJSZA OD ZERA');
  END IF;
